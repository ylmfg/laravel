...
<div class="container clearfix">
    ...
    <div class="main-wrap">
        ...
        <div class="crumb-wrap">
            <div class="crumb-list"><i class="icon-font"></i><a href="{{url('index')}}">首页</a><span class="crumb-step">&gt;</span><a class="crumb-name" href="{{url('zuoping')}}">作品管理</a><span class="crumb-step">&gt;</span><span>商品属性</span></div>
        </div>
        <div class="result-wrap">
            <div class="result-content">
                ...
                @endif
                <form action="{{url('attribute')}}" method="post" id="myform" name="myform" enctype="multipart/form-data">
                    <table class="insert-tab" width="100%">
                        <tbody><tr>
                            <th width="120"><i class="require-red">*</i>属性类型：</th>
                            <td>
                                <select name="type_id" id="typeid" class="required">
                                    <option value="">请选择</option>
                                    @foreach ($goodType as $gt)

                                    @if(!empty($attribute->type_id))
                                    <option value="{{$gt->type_id}}" @if($gt->type_id==$attribute->type_id) selected @endif>{{$gt->type_name}}</option>
                                    @else
                            ...
                            <tr>
                                ...
                                <td>
                                    @if(!empty($attribute->attr_name))
                                      <input class="common-text required" id="title" name="attr_name" size="50" value="{{$attribute->attr_name}}" type="text">
                                      @else
                            ...
                            <tr>
                                <th><i class="require-red">*</i>可选值列表：</th>
                                <td>
                                <textarea name="attr_value" rows="15" cols="120"></textarea>
                                </td>
                            </tr>
                            <tr>
                                <th></th>
...
<script>
    $(function(){
        var ff=$("input[name='attr_input_type']").val();
           if(ff!=2){
              $("textarea[name='attr_value']").css('background','#C0C0C0');
           }else{
              $("textarea[name='attr_value']").css('background','#FFFFFF');
           }
